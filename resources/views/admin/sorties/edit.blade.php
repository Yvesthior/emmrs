        ...
        <div class="col-lg-12">
            <div class="panel panel-default">
                <div class="panel-heading">
                    {{ trans('global.edit') }} {{ trans('cruds.sorty.title_singular') }}
                </div>
                <div class="panel-body">
                    <form method="POST" action="{{ route("admin.sorties.update", [$sorty->id]) }}" enctype="multipart/form-data">
                        @method('PUT')
                        @csrf
                        <div class="form-group {{ $errors->has('equipement') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="equipement_id" id="equipement_id" required>
                                @foreach($equipements as $id => $entry)
                                    <option value="{{ $id }}" {{ (old('equipement_id') ? old('equipement_id') : $sorty->equipement->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group {{ $errors->has('quantite') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="number" name="quantite" id="quantite" value="{{ old('quantite', $sorty->quantite) }}" step="1" required>
                        ...
                        <div class="form-group {{ $errors->has('date_sortie') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control date" type="text" name="date_sortie" id="date_sortie" value="{{ old('date_sortie', $sorty->date_sortie) }}" required>
                        ...
                        <div class="form-group {{ $errors->has('rubrique') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="text" name="rubrique" id="rubrique" value="{{ old('rubrique', $sorty->rubrique) }}" required>
                        ...
                        <div class="form-group {{ $errors->has('destination') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="destination_id" id="destination_id" required>
                                @foreach($destinations as $id => $entry)
                                    <option value="{{ $id }}" {{ (old('destination_id') ? old('destination_id') : $sorty->destination->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group {{ $errors->has('observation') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="text" name="observation" id="observation" value="{{ old('observation', $sorty->observation) }}">
