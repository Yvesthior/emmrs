        ...
        <div class="col-lg-12">
            <div class="panel panel-default">
                <div class="panel-heading">
                    {{ trans('global.edit') }} {{ trans('cruds.incrementationMateriel.title_singular') }}
                </div>
                <div class="panel-body">
                    <form method="POST" action="{{ route("admin.incrementation-materiels.update", [$incrementationMateriel->id]) }}" enctype="multipart/form-data">
                        @method('PUT')
                        @csrf
                        <div class="form-group {{ $errors->has('materiel') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="materiel_id" id="materiel_id">
                                @foreach($materiels as $id => $entry)
                                    <option value="{{ $id }}" {{ (old('materiel_id') ? old('materiel_id') : $incrementationMateriel->materiel->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group {{ $errors->has('quantite') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="number" name="quantite" id="quantite" value="{{ old('quantite', $incrementationMateriel->quantite) }}" step="1">
                        ...
                        <div class="form-group {{ $errors->has('date') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control date" type="text" name="date" id="date" value="{{ old('date', $incrementationMateriel->date) }}">
                            @if($errors->has('date'))
                        ...
                        <div class="form-group {{ $errors->has('provenance') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="provenance_id" id="provenance_id">
                                @foreach($provenances as $id => $entry)
                                    <option value="{{ $id }}" {{ (old('provenance_id') ? old('provenance_id') : $incrementationMateriel->provenance->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group {{ $errors->has('prix_unitaire') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="number" name="prix_unitaire" id="prix_unitaire" value="{{ old('prix_unitaire', $incrementationMateriel->prix_unitaire) }}" step="1">
                        ...
                        <div class="form-group {{ $errors->has('date_peremption') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="text" name="date_peremption" id="date_peremption" value="{{ old('date_peremption', $incrementationMateriel->date_peremption) }}">
                        ...
                        <div class="form-group {{ $errors->has('fabriquant') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="fabriquant_id" id="fabriquant_id">
                                @foreach($fabriquants as $id => $entry)
                                    <option value="{{ $id }}" {{ (old('fabriquant_id') ? old('fabriquant_id') : $incrementationMateriel->fabriquant->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group {{ $errors->has('destination') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="destination_id" id="destination_id">
                                @foreach($destinations as $id => $entry)
                                    <option value="{{ $id }}" {{ (old('destination_id') ? old('destination_id') : $incrementationMateriel->destination->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                                @endforeach
                            </select>
