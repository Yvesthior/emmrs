        ...
        <div class="col-lg-12">
            <div class="panel panel-default">
                <div class="panel-heading">
                    {{ trans('global.create') }} {{ trans('cruds.sortiesMedicament.title_singular') }}
                </div>
                <div class="panel-body">
                    <form method="POST" action="{{ route("admin.sorties-medicaments.store") }}" enctype="multipart/form-data">
                        @csrf
                        <div class="form-group {{ $errors->has('medicament') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="medicament_id" id="medicament_id" required>
                                @foreach($medicaments as $id => $entry)
                                    <option value="{{ $id }}" {{ old('medicament_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group {{ $errors->has('total_unite') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="number" name="total_unite" id="total_unite" value="{{ old('total_unite', '1') }}" step="1">
                        ...
                        <div class="form-group {{ $errors->has('upc') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="number" name="upc" id="upc" value="{{ old('upc', '') }}" step="1" required>
                            @if($errors->has('upc'))
                        ...
                        <div class="form-group {{ $errors->has('rubrique_sortie') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="text" name="rubrique_sortie" id="rubrique_sortie" value="{{ old('rubrique_sortie', '') }}" required>
                        ...
                        <div class="form-group {{ $errors->has('cout') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="number" name="cout" id="cout" value="{{ old('cout', '') }}" step="1">
                            @if($errors->has('cout'))
